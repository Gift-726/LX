...
let isGmail: boolean = false;
let initializationPromise: Promise<boolean> | null = null;

type EmailType = 'registration' | 'login' | 'password-reset';
...
const createTransporter = async (forceEthereal: boolean = false): Promise<boolean> => {
  ...
  if (forceEthereal) {
    try {
      ...
      console.error('❌ Failed to create Ethereal account:', errorMessage);
      return false;
    }
  }
  ...
  const gmailConfigs: GmailConfig[] = [
    {
...
const ensureTransporterReady = async (): Promise<boolean> => {
...
const getEmailTemplate = (code: string, type: EmailType = 'password-reset'): EmailTemplate => {
  const templates = {
    'registration': {
...
${emailTemplate.html.match(/<h1>(.*?)<\/h1>/)?.[1] || ''}
...
© ${new Date().getFullYear()} LX. All rights reserved.
    ...
    return { 
      success: false, 
      error: errorMessage,
      details: (error as any).code || (error as any).response || 'Unknown error'
    };
  }
};
...
export type { EmailType, SendEmailResult };
